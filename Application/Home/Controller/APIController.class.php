...
class APIController extends Controller {
	...
	*/
	...
	public function InsertData($TTdata=''){
		...
		$TMData_SQL="";//trapmodel 更新状态
		$TMData_time_SQL = "";//trapmodel 更新状态
		$PER_STR="";
		if($Result != ""){
			...
			foreach ($Result_p_array as $parrayData) {
				...
				foreach ($Parray_d_arr as $parray) {
					...
					if(strtolower($Result_array[0])=="cms"){
						...
						/////////////////////////////////////////////////////////////////
						$TM_VALUE="-1";
						if($c_array["TemState"]=="0"){
							$TM_VALUE=$c_array["ExLevel"];
						}
						$TMData_SQL .= "  WHEN  '". $c_array["TrapNo"]."' THEN ".$TM_VALUE."  ";
						$TMData_time_SQL .= "  WHEN  '". $c_array["TrapNo"]."' THEN '".date('Y-m-d H:i:s')."'  ";
			...
			if($arr_all_result!=null){
				...
				////////////TrapModel更新
				$TrapModelDao = M();
				$TrapModelDao->query("UPDATE trapmodel SET `status` = CASE  trapno ".$TMData_SQL." end,`statusTime` = CASE trapno ".$TMData_time_SQL." END Where companyID=".$comid);
				/*
